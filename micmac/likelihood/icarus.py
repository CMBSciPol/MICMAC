...
from micmac.noise.noisecovar import get_BtinvN, get_inv_BtinvNB, get_Wd
    ...
    def perform_harmonic_MH(
        ...
        mcmc_obj = numpyro.infer.mcmc.MCMC(
            MultiStepsMetropolisHastings(
                log_proba=self.icarus_harmonic_marginal_probability, covariance_dict=covariance_dict
            ),
            num_warmup=0,
            num_samples=self.number_iterations_sampling - self.number_iterations_done,
            progress_bar=True,
    ...
    def perform_Gibbs_sampling(
        ...
        **dictionnary_additional_parameters,
        ...
        def all_sampling_steps(carry, iteration):
            ...
            concat_cov_matrix_sqrt = get_sqrt_reduced_matrix_from_matrix_jax(
                carry['redcom_cov_matrix_sample'].reshape(
                    (self.lmax - self.lmin + 1, self.n_components * self.nstokes, self.n_components * self.nstokes)
                )
            )
            redcom_cov_matrix_sqrt = concat_cov_matrix_sqrt.reshape(
                (self.lmax - self.lmin + 1, self.n_components, self.nstokes, self.n_components, self.nstokes)
            )
            ...
            #     noise_c_ell = get_inv_BtinvNB_c_ell(self.freq_noise_c_ell, mixing_matrix_sampled.mean(axis=2))
            #     ## Getting N_c^{-1} for the harmonic noise covariance
            #     red_noise_c_ell = np.einsum(
            #         'fnl,sk->fnlsk', noise_c_ell, jnp.eye(2)
            #         )
            #     concatenated_red_inv_noise_c_ell = jnp.linalg.pinv(
            #         concatenate_reduced_multi_components_matrix(red_noise_c_ell)
            #     )  # [self.lmin:]
            #     concatenated_red_preconditioner_s = jnp.linalg.pinv(
            #         jnp.eye(self.nstokes * self.n_components)
            #         + jnp.einsum('lij,ljk,lkm->lim', concatenated_red_cov_matrix_sqrt, concatenated_red_inv_noise_c_ell, concatenated_red_cov_matrix_sqrt)
            #     )

            #     precond_func_s_c = lambda x: maps_x_red_covariance_cell_JAX(
            #         x.reshape((self.nstokes * self.n_components, self.n_pix)),
            #         concatenated_red_preconditioner_s,
            #         nside=self.nside,
            #         lmin=self.lmin,
            #         n_iter=self.n_iter,
            #     ).ravel()
            ...
            new_carry['wiener_filter_term'] = sampling_func_WF(
                s_GLS, redcom_cov_matrix_sqrt, redcom_N_inv, initial_guess=initial_guess_WF, precond_func=precond_func_s
            )
            ...
            # Sampling step 3: sampling of foreground covariance Sf TODO: WIP
