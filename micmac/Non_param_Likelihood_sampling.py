...
class MICMAC_Sampler(Sampling_functions):
    def __init__(self, nside, lmax, nstokes, 
                 ...
                 n_walkers_Metropolis=1, step_size_B_f=10**(-5), step_size_r=10**(-4),
        ...
        super(MICMAC_Sampler,self).__init__(nside=nside,lmax=lmax,nstokes=nstokes,lmin=lmin,
                                            ...
                                            n_iter=n_iter, limit_iter_cg=limit_iter_cg, tolerance_CG=tolerance_CG, mask=mask)
        ...
        # Sampling parameters
        self.overrelax = float(overrelax) # Overrelaxation parameter for the CG
    ...
    # def compute_covariance(self, iteration, current_sample, mean_sample_t, previous_covariance, all_samples):
    #     return jlx.cond(jnp.size(jnp.shape(mean_sample_t))!=1, 
    #                     self.compute_covariance_nd, 
    #                     self.compute_covariance_1d,
    #                     iteration, all_samples)

    #     # return jlx.cond(jnp.size(jnp.shape(current_sample))!=1, 
    #     #                 self.compute_covariance_nd, 
    #     #                 self.compute_covariance_1d,
    #     #                 iteration, all_samples)

    # def get_new_covariance_recursion(self, iteration, current_sample, mean_sample_t, previous_covariance, all_samples=jnp.empty(0)):

    #     # dimension_sample = jnp.where(jnp.size(jnp.shape(mean_sample_t))!=1, jnp.shape(mean_sample_t), 1)
    #     dimension_sample = jnp.shape(mean_sample_t)

    #     mean_sample_t_m1 = np.copy(mean_sample_t)
    #     new_mean_sample_t = (iteration/(iteration+1))*mean_sample_t_m1 + jnp.ravel(current_sample,order='F')/(iteration+1)
    #     new_covariance_matrix = ((iteration-1)/iteration)*previous_covariance + (self.scale_param/dimension_sample/iteration)*(jnp.einsum('i,j->ij', jnp.ravel(current_sample,order='F'), jnp.ravel(current_sample,order='F'))
    #                             + iteration*jnp.einsum('i,j->ij',mean_sample_t_m1,mean_sample_t_m1)
    #                             - (iteration+1)*jnp.einsum('i,j->ij',new_mean_sample_t,new_mean_sample_t)
    #                             + self.epsilon_cov*jnp.eye(dimension_sample))
    #     # return mean_sample_t, new_covariance_matrix
    #     return new_covariance_matrix

    # def get_covariance_outside_AM(self, iteration, current_sample, mean_sample_t, previous_covariance, all_samples=jnp.empty(0)):
    #     return jnp.where(iteration < self.num_sample_AM,
    #                     previous_covariance,
    #                     self.get_new_covariance_recursion(iteration, current_sample, mean_sample_t, previous_covariance))
    
    # def get_sample_covariance_MH(self, iteration, current_sample, mean_sample_t, previous_covariance, all_samples=jnp.empty(0)):
    #     new_covariance_matrix = jlx.cond(iteration == self.num_sample_AM,
    #                                       self.compute_covariance,
    #                                       self.get_covariance_outside_AM,
    #                                     iteration, current_sample, mean_sample_t, previous_covariance, all_samples)
    #     mean_sample_t = (iteration/(iteration+1))*mean_sample_t + jnp.ravel(current_sample,order='F')/(iteration+1)
    #     return new_covariance_matrix
    #     # return mean_sample_t, new_covariance_matrix
    ...
    def perform_sampling(self, input_freq_maps, c_ell_approx, CMB_c_ell, init_params_mixing_matrix, 
        ...
        # jitted_get_conditional_proba_mixing_matrix_v2_JAX = jax.jit(self.get_conditional_proba_mixing_matrix_v2_slow_JAX_unpix)
        print("Using UNPIX slow version of B_f sampling !", flush=True)
        if self.biased_version:
            print("Using biased version of mixing matrix sampling !!!", flush=True)
            jitted_func_to_use = jax.jit(self.get_biased_conditional_proba_mixing_matrix_v2_slow_JAX)
            # jitted_func_to_use = jax.jit(self.get_biased_conditional_proba_mixing_matrix_v2_slow_JAX_unpix)

        # jitted_get_conditional_proba_mixing_matrix_v1_JAX = jax.jit(self.get_conditional_proba_mixing_matrix_v1_slow_JAX_unpix_harm)
        if self.harmonic_correction:
            print("Using harmonic correction for mixing matrix sampling !!!", flush=True)
            jitted_func_to_use = jax.jit(self.get_conditional_proba_mixing_matrix_v1_slow_JAX_unpix_harm)
        else:
            jitted_func_to_use = jax.jit(self.get_conditional_proba_mixing_matrix_v2_slow_JAX_unpix)

        # raveled_indexes_B_f = self.indexes_free_Bf
        ...
        initial_covariance_step_size_B_f = self.covariance_step_size_B_f
        step_size_Bf = jnp.array(jnp.diag(jsp.linalg.sqrtm(initial_covariance_step_size_B_f)), dtype=jnp.float64)
        print('Step-size B_f', step_size_Bf, flush=True)
        ...
        def all_sampling_steps(carry, iteration):
            ...
            eta_maps_sample, WF_term_maps, fluct_maps, red_cov_matrix_sample, _all_r_samples, _all_B_f_samples, PRNGKey, old_s_c_sample = carry
            ...
            if self.sample_eta_B_f and not(self.biased_version):
                ...
                new_eta_maps_sample = self.get_sampling_eta_v2(red_cov_approx_matrix, BtinvNB, BtinvN_sqrt, 
                                                               subPRNGKey, map_random_x=map_random_x, map_random_y=map_random_y, 
                                                               suppress_low_modes=True)
                # eta_maps_sample = jitted_sample_eta(red_cov_approx_matrix, BtinvNB, BtinvN_sqrt, subPRNGKey, map_random_x=map_random_x, map_random_y=map_random_y, suppress_low_modes=True)

                eta_maps_sample = eta_maps_sample*self.overrelax + ((1-self.overrelax**2)**.5)*new_eta_maps_sample
            ...
            fluctuation_maps = self.get_fluctuating_term_maps(red_cov_matrix_sample, BtinvNB, BtinvN_sqrt, new_subPRNGKey, 
            ...
            # s_c_sample = wiener_filter_term + fluctuation_maps
            s_c_sample = wiener_filter_term + self.overrelax*(old_s_c_sample - wiener_filter_term) + ((1-self.overrelax**2)**.5)*fluctuation_maps
            ...
            if self.sample_C_inv_Wishart:
                ...
                new_subPRNGKey_2, red_cov_matrix_sample = jitted_get_inverse_wishart_sampling_from_c_ells(c_ells_Wishart, PRNGKey=new_subPRNGKey_2, q_prior=0, option_ell_2=2, tol=self.tolerance_CG)
                ...
                # mean_sample_r = _all_r_samples.sum()/(iteration+1)
                # self.step_size_r = jnp.sqrt(self.get_sample_covariance_MH(iteration, _all_r_samples[iteration], mean_sample_r, self.step_size_r**2, _all_r_samples))
            ...
            # red_cov_matrix_sample = jnp.where(red_cov_matrix_sample.shape[0] == self.lmax + 1, red_cov_matrix_sample[self.lmin:], red_cov_matrix_sample)
            ...
            if self.sample_eta_B_f:
                ...
                step_size_Bf = jnp.array(jnp.diag(jsp.linalg.sqrtm(self.covariance_step_size_B_f)), dtype=jnp.float64)
                if self.use_automatic_step_size:
                    covariance_matrix_B_f = jnp.where(iteration < self.num_sample_AM, 
                                                    initial_covariance_step_size_B_f,
                ...
                # else:
                #     covariance_matrix_B_f = self.covariance_step_size_B_f
                # mean_sample_B_f = _all_B_f_samples.sum()/(iteration+1)
                # self.covariance_step_size_B_f = self.get_sample_covariance_MH(iteration, _all_B_f_samples[iteration], mean_sample_B_f, self.covariance_step_size_B_f, _all_B_f_samples)
                
                # step_size_Bf = jnp.sqrt(jnp.diag(covariance_matrix_B_f))
                ...
                #                                           full_data_without_CMB=full_data_without_CMB, modified_sample_eta_maps=eta_maps_sample, 
                #                                           red_cov_approx_matrix=red_cov_approx_matrix)
                # new_subPRNGKey_3, params_mixing_matrix_sample = separate_single_MH_step(random_PRNGKey=new_subPRNGKey_3, old_sample=params_mixing_matrix_sample, 
                #                                           step_size=step_size_Bf, log_proba=jitted_func_to_use,
                #                                           full_data_without_CMB=full_data_without_CMB, modified_sample_eta_maps=eta_maps_sample, 
                #                                           red_cov_approx_matrix=red_cov_approx_matrix)
                new_subPRNGKey_3, params_mixing_matrix_sample = separate_single_MH_step_index(random_PRNGKey=new_subPRNGKey_3, old_sample=params_mixing_matrix_sample, 
                                                          step_size=step_size_Bf, log_proba=jitted_func_to_use, 
                                                          indexes_Bf=self.indexes_free_Bf,
                                                          full_data_without_CMB=full_data_without_CMB, modified_sample_eta_maps=eta_maps_sample, 
                                                          red_cov_approx_matrix=red_cov_approx_matrix)
                # time_sampling_Bf = (time.time()-time_start_sampling_Bf)/60
                # ravel_params = jnp.copy(params_mixing_matrix_sample).ravel(order='F')
                # ravel_params = ravel_params.at[self.indexes_free_Bf].set(partial_params_mixing_matrix_sample)
                # params_mixing_matrix_sample = params_mixing_matrix_sample.at[:,:].set(ravel_params.reshape((self.number_frequencies-len_pos_special_freqs,number_correlations-1),order='F'))
            ...
            new_carry = (eta_maps_sample, wiener_filter_term, fluctuation_maps, red_cov_matrix_sample, _all_r_samples, _all_B_f_samples, PRNGKey, s_c_sample)
        ...
        all_B_f_samples = jnp.zeros((actual_number_of_iterations+1, dimension_param_B_f))
    ...
    # def restart_sampling(self, number_iterations_to_perform, input_freq_maps, c_ell_approx, all_samples_params_mixing_matrix, 
    #                      initial_wiener_filter_term=jnp.empty(0), initial_fluctuation_maps=jnp.empty(0),
    #                      CMB_c_ell = jnp.empty(0), all_r_samples = jnp.empty(0), initial_guess_r=0, 
    #                      theoretical_r0_total=jnp.empty(0), theoretical_r1_tensor=jnp.empty(0)):
    #     """ 
    ...
    #     if self.num_sample_AM < self.number_iterations_done:
    #         self.num_sample_AM = -1
    ...
    #     init_params_mixing_matrix = all_samples_params_mixing_matrix[-1]
        

    #     self.perform_sampling(input_freq_maps, c_ell_approx, init_params_mixing_matrix, 
    #                      initial_wiener_filter_term, initial_fluctuation_maps,
    #                      CMB_c_ell, initial_guess_r, 
