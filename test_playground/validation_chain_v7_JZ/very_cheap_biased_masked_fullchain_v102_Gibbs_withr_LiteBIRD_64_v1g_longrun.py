...
file_ver = 'biased_masked_full_v102_Gchain_SO_64_v1e' # -> biased LiteBIRD + r=1e-2 + 2000 iterations + biased_v1d_LiteBIRD + w/o restrict_to_mask + unmasked ; C_approx only lensing
file_ver = 'biased_masked_full_v102_Gchain_SO_64_v1f' # -> biased LiteBIRD + r=0 + 2000 iterations + biased_v1e_LiteBIRD + w/o restrict_to_mask + unmasked ; C_approx only lensing
file_ver = 'biased_masked_full_v102_Gchain_SO_64_v1g' # -> biased LiteBIRD + r=0 + 2000 iterations + biased_v1f_LiteBIRD + w/o restrict_to_mask + unmasked ; C_approx only lensing
# -> TODO !!!
reduction_noise = 1
factor_Fisher = 1
...
path_Fisher = path_home_test_playground + 'Fisher_matrix_{}_EB_model_d0s0_noise_True_seed_42_lmin2_lmax128.txt'
...
path_toml_file = directory_toml_file + 'biased_v1e_LiteBIRD.toml'
path_toml_file = directory_toml_file + 'biased_v1f_LiteBIRD.toml'
...
freq_inverse_noise_masked[:,:,mask!=0] = np.repeat(freq_inverse_noise.ravel(order='F'), nb_pixels_mask).reshape((MICMAC_obj.n_frequencies,MICMAC_obj.n_frequencies,nb_pixels_mask), order='C')
...
Fisher_matrix = np.loadtxt(path_Fisher.format(MICMAC_obj.instrument_name))
