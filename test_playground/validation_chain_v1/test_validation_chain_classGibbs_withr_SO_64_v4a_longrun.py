...
file_ver = 'full_Gchain_classGibbs_SO_noise_withr_v2a' #  Long run 30 iterations with chain-in-code with noise -- 0 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E + step-size r 10**(-4) + limit_iter_CG 2000 ; r=0.001, start=10**(-8)
...
file_ver = 'full_Gchain_classGibbs_SO_noise_withr_v2ab' #  Long run 300 iterations with chain-in-code with noise -- 0 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E + step-size r 10**(-4) + limit_iter_CG 2000 ; r=0.001, start=10**(-8)
only_select_Bmodes = False
no_Emodes_CMB = False
...
# file_ver = 'full_Gchain_classGibbs_Emodes_SO_noise_withr_v4b' #  Full long run 5 iterations with chain-in-code with noise and E-modes -- 0 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E + step-size r 10**(-4) + limit_iter_CG 2000 ; r=0.001, start=10**(-8)
# only_select_Bmodes = False
# no_Emodes_CMB = False
# sample_eta_B_f = True
...
number_iterations_sampling = 300 #30 #150
