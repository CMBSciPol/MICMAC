...
only_select_Bmodes = True
no_Emodes_CMB = False
# file_ver = 'full_Gchain_SO_noise_withr_v4c' #  Mini-test long run 5 iterations with chain-in-code -- 10 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E ; r=0.001, start=10**(-8)
# only_select_Bmodes = False
# no_Emodes_CMB = False

sample_eta_B_f = True
overrelaxation_param = 0
r_true = 0.001
# r_true = 10**(-4)
...
Full_Gibbs_Sampler = micmac.MICMAC_Sampler(nside, lmax, nstokes, np.array(instrument['frequency']), freq_inverse_noise, 
                 ...
                 sample_eta_B_f=sample_eta_B_f, overrelaxation_param=overrelaxation_param,
