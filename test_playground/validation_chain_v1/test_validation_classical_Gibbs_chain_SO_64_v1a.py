...
file_ver = 'full_classicalGchain_withoutEB_SO_noise_v1h'
with_cross_diag_terms = False
...
number_iterations_sampling = 25 #150
# number_iterations_sampling = 10000
number_iterations_sampling = 75 #150
...
params_mixing_matrix_sample = init_params.ravel(order='F') #+ np.random.uniform(low=-step_size_array,high=step_size_array, size=((number_frequencies-len_pos_special_freqs)*2))
...
kernel = micmac.MetropolisHastings_log(micmac.new_get_conditional_proba_full_likelihood_JAX_from_params, step_size=step_size_array.ravel(order='F'))
mcmc_kernel = numpyro.infer.MCMC(kernel, num_chains=n_walkers, num_warmup=num_warmup, num_samples=number_steps_sampler)

print(f"Starting iterations for version {file_ver} with a total of {number_iterations_sampling} iterations !", flush=True)
...
for iteration in range(number_iterations_sampling):
    ...
    if not(with_cross_diag_terms):
        c_ells_Wishart[nstokes:,...] = 0
    ...
    # all_numpyro_mixing_matrix_samples = micmac.get_sample_parameter(mcmc_kernel, params_mixing_matrix_sample.ravel(order='F'), random_PRNGKey=jax.random.PRNGKey(100+iteration), pos_special_freqs=mixing_matrix_obj.pos_special_freqs, fullsky_ver=fullsky_ver, slow_ver=slow_ver, param_dict=param_dict, full_data_without_CMB=full_data_without_CMB, modified_sample_eta_maps=eta_maps_sample, freq_inverse_noise=freq_inverse_noise, red_cov_approx_matrix=red_cov_approx_matrix, lmin=lmin, n_iter=n_iter, limit_iter_cg=limit_iter_cg, tolerance=tolerance_CG, with_prints=False)
    ...
    print("##### Not sampling B_f at iteration {}".format(iteration+1), flush=True)
    params_mixing_matrix_sample = params_mixing_matrix_sample.reshape((number_frequencies-len_pos_special_freqs,2), order='F')
...
param_dict['number_steps_sampler'] = number_steps_sampler
