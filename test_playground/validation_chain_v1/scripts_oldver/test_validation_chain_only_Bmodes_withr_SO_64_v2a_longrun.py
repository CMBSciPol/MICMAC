...
only_select_Bmodes = True
no_Emodes_CMB = False
file_ver = 'full_Gchain_onlyB_SO_noise_v3fb' # Test long run 5 iterations -- 10 Fisher start from params + sampling for r + C_approx only lensing ; minimal number of E-modes put to 0 artificially + step-size r 10**(-5) ; r=0.001, start=10**(-8)
only_select_Bmodes = True
no_Emodes_CMB = False
...
# number_iterations_sampling = 500
...
for iteration in range(number_iterations_sampling):
    ...
    if only_select_Bmodes:
        fluctuation_maps = micmac.maps_x_reduced_matrix_generalized_sqrt_sqrt(np.copy(fluctuation_maps), red_cov_select_Bmodes, lmin=lmin, n_iter=n_iter)#[1:,...]
