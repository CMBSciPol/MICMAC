...
file_ver = 'full_Gchain_classGibbs_Emodes_SO_noise_withr_v4b' #  Full long run 300 iterations with chain-in-code with noise and E-modes -- 0 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E + step-size r 10**(-4) + limit_iter_CG 2000 ; r=0.001, start=10**(-8)
file_ver = 'full_Gchain_classGibbs_Emodes_SO_noise_withr_v4c' #  Full long run 300 iterations with chain-in-code with lower noise and E-modes -- 0 Fisher start from params + with E (but no E CMB) + sampling for r instead of C + B lensing in C_approx and only tensor E + step-size r 10**(-4) + limit_iter_CG 2000 ; r=0.001, start=10**(-8)
