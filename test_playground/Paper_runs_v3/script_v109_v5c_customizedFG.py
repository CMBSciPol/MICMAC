...
parser.add_argument(
    'additional_toml', metavar='N', type=str, nargs='+', help='a toml file path to be added to the config'
)
...
if fgs_model == 'customized_parametric':
    ...
    print(
        '--> using customized parametric fgs model, built from ',
        fgs_model_init,
        'with spv spectral params: ',
        nside_spv_model,
    )
    ...
    print(
        '--> using customized non-parametric fgs model, built from ',
        fgs_model_init,
        'with spv scaling laws: ',
        nside_spv_model,
    )
...
print(f'r{MPI_rank} of {MPI_size} -- Launch', flush=True)
...
if stack_former_file_ver:
    assert (
        'former_file_ver' != ''
    ), 'Need to have former_file_ver in additional parameters to stack former file version !'
...
if path_cov_B_f_r == '':
    path_Fisher = (
        path_home_test_playground
        + f'Fisher_matrix_{MICMAC_obj.instrument_name}_EB_model_{fgs_model_}_noise_True_seed_42_lmin2_lmax128.txt'
    )
    try:
        Fisher_matrix = np.loadtxt(path_Fisher)
    except:
        print('Fisher matrix not found !', flush=True)
        Fisher_matrix = np.loadtxt(
            path_home_test_playground
            + f'Fisher_matrix_{MICMAC_obj.instrument_name}_EB_model_d0s0_noise_True_seed_42_lmin2_lmax128.txt'
        )
else:
...
if fgs_model_ == 'customized_parametric':
    np.random.seed(seed_realization_input)
    nside_map = MICMAC_obj.nside
    my_sky, _ = micmac.parametric_sky_customized(fgs_model_init, nside_map, nside_spv_model)
    ...
    freq_maps_fgs_denoised = micmac.fgs_freq_maps_from_customized_model_nonparam(
        nside_map, nside_spv_model, instrument, fgs_models=fgs_model_init, idx_ref_freq=idx_ref_freq, return_mixing_matrix=False
    )
elif fgs_model_ != '':
    np.random.seed(seed_realization_input)
...
try:
    ...
    print(
        'Fisher matrix not invertible or scipy.linalg.sqrtm not working on GPU ! Taking only sqrt of diagonal elements instead !',
        flush=True,
    )
...
if not (MICMAC_obj.classical_Gibbs):
    ...
    MICMAC_obj.covariance_B_f = (
        np.copy(np.linalg.inv(Fisher_matrix))[:-1, :-1] / factor_Fisher
    )  # TODO: extend to case of many spv params
...
) = MICMAC_obj.generate_input_freq_maps_from_fgs(freq_maps_fgs, return_only_freq_maps=False)
...
print(fr'First guess from {sigma_gap} $\sigma$ Fisher !', f'rank {MPI_rank} over {MPI_size}', flush=True)
...
if initial_guess_B_f is None:
    ...
    first_guess = first_guess.at[MICMAC_obj.indexes_free_Bf].set(
        ...
        + step_size_B_f[MICMAC_obj.indexes_free_Bf]
        * np.random.uniform(low=-sigma_gap, high=sigma_gap, size=(dimension_free_param_B_f,))
    )
...
if MICMAC_obj.use_binning:
    ...
    MICMAC_obj.maximum_number_dof = int(
        MICMAC_obj.bin_ell_distribution[-1] ** 2 - MICMAC_obj.bin_ell_distribution[-2] ** 2
    )
...
if former_file_ver != '':
    ...
    new_noise_map = dict_all_params['input_noise_map']
...
if use_last_sample:
    dict_last_sample = jnp.load(
        directory_save_file + file_ver_last_sample + f'_{MPI_rank}_{MPI_size}' + '_last_sample.npz'
    )
    ...
    CMB_c_ell = np.zeros_like(c_ell_approx)
    CMB_c_ell[:, MICMAC_obj.lmin :] = micmac.get_c_ells_from_red_covariance_matrix(
        dict_last_sample['red_cov_matrix_sample']
    )
...
if MICMAC_obj.len_params != init_params_mixing_matrix.shape[0]:
    print(f'Expanding init_params_mixing_matrix from shape {init_params_mixing_matrix.shape[0]}!!', flush=True)
    # expend_factor = dimension_free_param_B_f//step_size_B_f.shape[0]
    expend_factor = MICMAC_obj.len_params // init_params_mixing_matrix.shape[0]
    # step_size_B_f = np.repeat(step_size_B_f, expend_factor)
    init_params_mixing_matrix = np.broadcast_to(
        init_params_mixing_matrix, (expend_factor, init_params_mixing_matrix.shape[0])
    ).ravel(order='F')
...
print(
    f'End of iterations in {time_full_chain} minutes, saving all files !',
    f'rank {MPI_rank} over {MPI_size}',
    flush=True,
)
...
if former_file_ver != '' and stack_former_file_ver:
    ...
    if MICMAC_obj.sample_r_Metropolis:
        all_r_samples = np.hstack(
            [dict_all_params['all_r_samples'].squeeze().ravel(), all_r_samples[1:].squeeze().ravel()]
        )
    elif MICMAC_obj.sample_C_inv_Wishart:
        all_cell_samples = np.hstack([dict_all_params['all_cell_samples'], all_cell_samples[1:]])

    all_params_mixing_matrix_samples = np.vstack(
        [dict_all_params['all_params_mixing_matrix_samples'], all_params_mixing_matrix_samples[1:]]
    )
...
print('FINAL SAVE - #### initial_noise_data :', initial_noise_map_path, f'rank {MPI_rank} over {MPI_size}', flush=True)
np.save(initial_noise_map_path, new_noise_map)
...
if MICMAC_obj.sample_C_inv_Wishart:
    ...
    print(
        'FINAL SAVE - #### params_mixing_matrix :',
        all_cell_samples_path,
        f'rank {MPI_rank} over {MPI_size}',
        flush=True,
    )
...
if MICMAC_obj.save_s_c_spectra:
    ...
    print(
        'FINAL SAVE - #### all_samples_s_c_spectra :',
        all_s_c_spectra_path,
        f'rank {MPI_rank} over {MPI_size}',
        flush=True,
    )
...
print(
    'FINAL SAVE - #### params_mixing_matrix :',
    all_params_mixing_matrix_samples_path,
    f'rank {MPI_rank} over {MPI_size}',
    flush=True,
)
...
print(
    'FINAL SAVE - Last PRNG key :',
    directory_save_file + file_ver + '_last_PRNGkey.npy',
    f'rank {MPI_rank} over {MPI_size}',
    flush=True,
)
...
if save_last_sample:
    print(
        'FINAL SAVE - Last sample :',
        directory_save_file + file_ver + '_last_sample.npy',
        f'rank {MPI_rank} over {MPI_size}',
        flush=True,
    )
    jnp.savez(directory_save_file + file_ver + '_last_sample.npz', **MICMAC_obj.last_sample)

print(f'End of saving in {(time.time()-time_saving)/60} minutes !', f'rank {MPI_rank} over {MPI_size}', flush=True)
