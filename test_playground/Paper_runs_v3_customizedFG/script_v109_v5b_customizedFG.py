...
if fgs_model_ == 'd1s1':
    ...
    my_sky = micmac.d1s1_sky_customized(nside_map, nside_spv)    #TODO: this could be done better with nside_spv passed in params file
    # my_freq_maps = get_observation_customized(instr, my_sky, noise=False, nside=nside_map)
    freq_maps_fgs_denoised = micmac.get_observation_customized(instrument, my_sky, nside=MICMAC_obj.nside, noise=False)[:, 1:, :]   # keep only Q and U
else:
